...
impl ComponentManifestBuilder {
    /// Create a new builder pre-configured for assistant providers.
    ///
    /// Sets `component_type` to `"assistant_provider"`.
    pub fn new_for_assistant() -> Self {
        Self {
            name: None,
            component_type: "assistant_provider".to_string(),
            version: None,
            description: None,
            license: None,
            compatibility: None,
    ...
    pub fn to_toml(&self) -> crate::error::Result<String> {
        let name = self.name.as_deref().ok_or_else(|| {
            crate::error::AssistantSdkError::ManifestValidation(
        ...
        if !is_valid_semver(version) {
            return Err(crate::error::AssistantSdkError::ManifestValidation(
                lifesavor_agent::registry::manifest::ManifestValidationError {
                    file_path: "component-manifest.toml".to_string(),
                    field_name: "version".to_string(),
        ...
        if self.component_type != "assistant_provider" {
            return Err(crate::error::AssistantSdkError::ManifestValidation(
                lifesavor_agent::registry::manifest::ManifestValidationError {
                    file_path: "component-manifest.toml".to_string(),
                    field_name: "type".to_string(),
                    description: format!(
                        "component type '{}' does not match Assistant SDK domain (expected 'assistant_provider')",
                        self.component_type
                    ),
                },
            ));
        ...
        toml::to_string_pretty(&manifest).map_err(|e| {
            crate::error::AssistantSdkError::Io(std::io::Error::other(
...
mod tests {
    ...
    #[test]
    fn test_default_assistant_manifest() {
        let toml_str = ComponentManifestBuilder::new_for_assistant()
            .name("my-assistant-provider")
            .version("0.1.0")
            .description("An assistant provider")
            .license("MIT")
            .to_toml()
            .unwrap();
        assert!(toml_str.contains("name = \"my-assistant-provider\""));
        assert!(toml_str.contains("type = \"assistant_provider\""));
        assert!(toml_str.contains("version = \"0.1.0\""));
    }

    #[test]
    fn test_missing_name_errors() {
        let result = ComponentManifestBuilder::new_for_assistant()
            .version("0.1.0")
            .to_toml();
    ...
    fn test_badge_generation() {
        let toml_str = ComponentManifestBuilder::new_for_assistant()
            .name("my-assistant")
            .version("1.0.0")
            .badge("build", "my-assistant")
            .to_toml()
            .unwrap();
        assert!(toml_str.contains("developer.stage.lifesavor.ai/badges/my-assistant/build.svg"));
    }
}

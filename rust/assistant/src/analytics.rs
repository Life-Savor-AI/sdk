...
///
/// ```rust,ignore
/// use lifesavor_assistant_sdk::analytics::{AnalyticsReporter, AnalyticsEvent, AnalyticsEventType};
///
...
impl AnalyticsReporter {
    ...
    /// Flush all pending events to the Developer Portal API.
    ///
    /// Uses a mock/no-op HTTP client for now.
    pub async fn flush(&mut self) -> Result<(), AnalyticsError> {
        ...
        self.failed_buffer.extend(events_to_send.drain(..to_keep));

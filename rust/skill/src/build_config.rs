...
//! commands, environment variables, and output directory for skill providers.
...
/// should build and test a skill provider component.
...
mod tests {
    ...
    #[test]
    fn test_default_skill_build_config() {
        let builder = BuildConfigBuilder::new_for_skill();
    ...
    fn test_custom_env_vars_and_steps() {
        let yaml = BuildConfigBuilder::new_for_skill()
    ...
    fn test_to_file_writes_yaml() {
        let dir = std::env::temp_dir().join("build_config_test_skill");
        let _ = std::fs::create_dir_all(&dir);
        let path = dir.join("lifesavor-build.yml");
        BuildConfigBuilder::new_for_skill()
            .to_file(&path)
            .unwrap();

...
impl ComponentManifestBuilder {
    /// Create a new builder pre-configured for system components.
    ///
    /// Sets `component_type` to `"system_component"`.
    pub fn new_for_system() -> Self {
        Self {
            name: None,
            component_type: "system_component".to_string(),
            version: None,
            description: None,
            license: None,
            compatibility: None,
    ...
    pub fn to_toml(&self) -> crate::error::Result<String> {
        ...
        if self.component_type != "system_component" {
            return Err(crate::error::SystemSdkError::ManifestValidation(
                crate::ManifestValidationError {
                    ...
                    description: format!(
                        "component type '{}' does not match System SDK domain (expected 'system_component')",
                        self.component_type
                    ),
                },
            ));
        ...
        toml::to_string_pretty(&manifest).map_err(|e| {
            crate::error::SystemSdkError::Io(std::io::Error::other(
...
fn is_valid_semver(version: &str) -> bool {
    ...
    // Validate pre-release if present
...
mod tests {
    ...
    #[test]
    fn test_system_manifest_builder() {
        let toml_str = ComponentManifestBuilder::new_for_system()
            .name("my-tts-component")
            .version("0.1.0")
            .description("A TTS system component")
            .license("MIT")
            .to_toml()
            .unwrap();
        assert!(toml_str.contains("name = \"my-tts-component\""));
        assert!(toml_str.contains("type = \"system_component\""));
    ...
    fn test_badge_generation() {
        let toml_str = ComponentManifestBuilder::new_for_system()
            .name("my-component")
            .version("1.0.0")
            .badge("build", "my-component")
            .badge("version", "my-component")
            .to_toml()
            .unwrap();
        assert!(toml_str.contains("developer.stage.lifesavor.ai/badges/my-component/build.svg"));
        assert!(toml_str.contains("developer.stage.lifesavor.ai/badges/my-component/version.svg"));
    }

    #[test]
    fn test_to_file_writes_toml() {
        let dir = std::env::temp_dir().join("component_manifest_test_system");
        let _ = std::fs::create_dir_all(&dir);
        let path = dir.join("component-manifest.toml");
        ComponentManifestBuilder::new_for_system()
            .name("test-component")
            .version("0.1.0")
            .to_file(&path)
            .unwrap();
        let content = std::fs::read_to_string(&path).unwrap();
        assert!(content.contains("type = \"system_component\""));

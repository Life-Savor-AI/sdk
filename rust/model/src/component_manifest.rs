...
impl ComponentManifestBuilder {
    ...
    ///
    /// Sets `component_type` to `"llm_provider"`.
    pub fn new_for_model() -> Self {
        Self {
            name: None,
            component_type: "llm_provider".to_string(),
            version: None,
            description: None,
            license: None,
            compatibility: None,
    ...
    pub fn to_toml(&self) -> crate::error::Result<String> {
        let name = self.name.as_deref().ok_or_else(|| {
            crate::error::ModelSdkError::ManifestValidation(
        ...
        if !is_valid_semver(version) {
            return Err(crate::error::ModelSdkError::ManifestValidation(
                lifesavor_agent::registry::manifest::ManifestValidationError {
                    file_path: "component-manifest.toml".to_string(),
                    field_name: "version".to_string(),
        ...
        if self.component_type != "llm_provider" {
            return Err(crate::error::ModelSdkError::ManifestValidation(
                lifesavor_agent::registry::manifest::ManifestValidationError {
                    file_path: "component-manifest.toml".to_string(),
                    field_name: "type".to_string(),
                    description: format!(
                        "component type '{}' does not match Model SDK domain (expected 'llm_provider')",
                        self.component_type
                    ),
                },
            ));
        ...
        toml::to_string_pretty(&manifest).map_err(|e| {
            crate::error::ModelSdkError::Io(std::io::Error::other(
...
mod tests {
    ...
    #[test]
    fn test_default_model_manifest() {
        let toml_str = ComponentManifestBuilder::new_for_model()
            .name("my-llm-provider")
            .version("0.1.0")
            .description("An LLM provider")
            .license("MIT")
            .to_toml()
            .unwrap();
        assert!(toml_str.contains("name = \"my-llm-provider\""));
        assert!(toml_str.contains("type = \"llm_provider\""));
        assert!(toml_str.contains("version = \"0.1.0\""));
    }

    #[test]
    fn test_missing_name_errors() {
        let result = ComponentManifestBuilder::new_for_model()
            .version("0.1.0")
            .to_toml();
    ...
    fn test_badge_generation() {
        let toml_str = ComponentManifestBuilder::new_for_model()
            .name("my-provider")
            .version("1.0.0")
            .badge("build", "my-provider")
            .to_toml()
            .unwrap();
        assert!(toml_str.contains("developer.stage.lifesavor.ai/badges/my-provider/build.svg"));
    }
}
